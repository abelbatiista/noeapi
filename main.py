...
def SelectPuesto(id):
    try:
        ...
        sql = f"""SELECT * FROM PUESTOS WHERE ID = {id}"""
...
@app.get("/InsertarUsuarios/{nombre}/{apellido}/{correo}/{password}/{edad}/{pais}/{telefono}")
...
@app.get("/RegistrarCategoria/{nombre}")
...
@app.get("/InsertarPuesto/{categoria}/{tipo}/{company}/{url}/{posicion}/{ubicacion}/{descripcion}/{email}")
...
@app.get("/NombreCategorias")
def NombreCategorias():
    try:
        global counter
        ...
        cursor.execute(sql)
        result = cursor.fetchall()
        return dict(NombreCategoria=result)
    except:
        return dict(Error="Fallo la ejecucion")
...
@app.get("/FiltroPuestos/{id}")
def EditarPuesto(id:int):
    try:
        global counter
        sql = """SELECT * FROM PUESTOS WHERE ID = %s"""
        campos = (id)
        connection = myDB
        cursor = connection.cursor()
        cursor.execute(sql,campos)
...
@app.get("/selectPuesto/{id}")
def _selectPuesto(id: int):
    try:
        condicion = SelectPuesto(id)
        if (condicion[0] != 0):
            array = condicion[1]
            return {
                "ID": array[0],
